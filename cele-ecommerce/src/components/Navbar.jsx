    ...
    <header className="hidden md:flex font-serif justify-between  items-center bg-white text-black py-1 px-12 shadow-lg border-b-2 border-blue-600">
       <div> <img src={logo} alt="logo" className='w-40 h-10 lg:w-60 lg:h-20 rounded-2xl lg:p-2'/></div>
        <nav className='flex space-x-4 font-bold '>
            ...
            <Link to= "/ProductsPage" className='hover:text-red-400 text-blue-600'>Products</Link>
            <Link to= "/About" className='hover:text-red-400 text-blue-600'>About Us</Link>
            <Link to= "/Contact" className='hover:text-red-400 text-blue-600'>Contact</Link>
        </nav>
        <div className="flex space-x-8">
            <button className='flex items-center justify-center space-x-1 hover:text-gray-400'>
               <User className='items-center'/>
              <div>
                <h5 className="hidden lg:flex lg:text-[14px] font-bold">My accout</h5>
                <p className="hidden text-[12px] lg:flex lg:text-[14px] font-bold">Login</p>
              </div>
            </button>
